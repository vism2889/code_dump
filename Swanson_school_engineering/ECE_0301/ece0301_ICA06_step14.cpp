/*
	...
	DATE: 09/16/2020
 	FILENAME: ece0301_ICA06_step14.cpp
	DESCRIPTION:
...
int main() {
	...
	// declares arrays to hold the values from the inputfile for 'A' and 'b'
	double a[DIM][DIM];
	double b[DIM];
	
	// call to getN to grab the dimensions and the values required values from the 
	// input file.
	getN(fileIn, a, b);
	
	// call to det() function to calculate the determinant of 'A'
	double detVal = det(a, DIM);
	
	// declares a 2D array to store a matrix with replaced column in it.
	double colRep[DIM][DIM];
	
	// declares an array to store the solution vector in.
	double solVect[DIM];
	
	// calls made to colReplace() function and det() to find the 
	// values in the solution vector.
	for (int i = 0;i < DIM; i++){
		colReplace(a,b,colRep,i);
		solVect[i] = det(colRep, DIM)/det(a,DIM);
	}
	
	// makes a call to the displayEquation() function to display all of the 
	// equation information to the output file
	displayEquation(DIM, a,b, fileOut, solVect);
...
void displayEquation(int N, double arrayA[][DIM], double arrayB[DIM], ofstream &fileOut, double arrayX[DIM]){
	...
	fileOut << endl << "Solution: x =" << endl;
	for (int i = 0; i < N; i++){
		fileOut << "[" << setw(10) << arrayX[i] << "]" << endl;
	}
	fileOut << endl;
	
	// declares an array variable to store the vectormatrix product in
	double vectorProduct[DIM];
	
	// inititalizes the array variable to store the vectormatrix product in
	matrixMultiply(arrayA, arrayX, vectorProduct);
	
	// outputs the check for the solution formatted exactly the same as
	// the output equation above.
	fileOut << "Checking Solution: A * x = b" << endl;
	for (int i = 0; i < N; i++){
		fileOut << "[";
		for (int j = 0; j < N; j++){
			fileOut << setw(10) << arrayA[i][j];
		...
		fileOut << setw(10) << vectorProduct[i];
		fileOut << "]" << endl;;
		 
	}
	
	// outputs the calculated error between the solution and the correct solution
	fileOut << endl << "Error in RHS values:" << endl;
	for (int i = 0; i < N; i++){
		fileOut << "[" << setw(10) << vectorProduct[i] - arrayB[i] << "]" << endl;
	}
...
void matrixDisplay( double array2[][DIM], int N, ofstream &fileOut){
	// this function is used to display the first N rows and cols of a matrix 
	// to an output file.
