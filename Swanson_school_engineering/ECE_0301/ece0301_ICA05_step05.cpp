/*
	...
	DATE: 09/07/2020
 	FILENAME: ece0301_ICA05_step05.cpp
	DESCRIPTION:
...
int main(){
	...
	fileOut << "M" << numFromFile << " = "  << maxTerm(numFromFile) << endl;
...
string minTerm(int num){
	...
	// if - else statments to translate the conveted bits into the variable string
...
string maxTerm(int num){
	...
	// Edge case to ensure the number from the input file is between 0-7
	if (num > 7 || num < 0){
		cout << "ERROER! Invalid minterm index." << endl;
	}
...
void displayTruthTable(ofstream &outputFile, bool andOR){
	...
	if (andOR){
		...
		outputFile << "Truth table for AND gate." << endl << endl;
	...
	// which displays the truth table for a three input AND gate in the output file.
		...
		outputFile << "Truth table for OR gate." << endl << endl;
		outputFile << "x\t\ty\t\tz\t\tf(x,y,z)" << endl;
		outputFile << "--------------------------------" << endl;
