...
int main(){
	...
	fileOut << "f" << numFromFile << " = " << csop(numFromFile) << endl << endl;
	//fileOut << csop(numFromFile) << endl << endl;
...
string csop(int num){
	// takes an int between 0 and 255 and returns a string representation 
	// of that value in  CSOP form
	...
	// for loop to find each of the 8 bits in the num variable and 
	// then call the minterm function on those bits.
	for (int i = 7; i >= 0;i--){
		// finds bit corresponding to place i
		int x = (num >> i) %2;
		// if bit is a minterm it calls the minterm function to
		// return the string represntation of it.
		if (x == 1){
			
			// if - else to format string output
			if (i != 0){
...
bool csop(bool x, bool y, bool z, int num){
	// Takes 3 bool values corresponding to parts of a minterm
	// and one int between 0-255
...
bool minTerm(bool x, bool y, bool z, int num){
	...
	// indicated by the integer input variable num.
	if (k == 0){
		x = !x; 
	}
	else {
...
bool maxTerm(bool x, bool y, bool z, int num){
	...
	// if - else statments to find complemnted values of a maxterm as
	// indicated by the integer input variable num.
	if (k == 0){
		x = x; 
	}
