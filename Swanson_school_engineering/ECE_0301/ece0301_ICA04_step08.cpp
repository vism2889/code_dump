/*
	...
	DATE: 09/04/2020
 	FILENAME: ece0301_ICA04_step08.cpp
	DESCRIPTION:
...
int main(){
	...
	if (firstLine == "Divider"){
		...
		lineOut << "-----" << endl << "Circuit #1 (Voltage Divider)" << endl;
		...
		inputFile >> r1;
		inputFile >> r2;
		inputFile >> multiplier; // reads multiplier value from the last line of the file
		
		// nested loops to generate new resistor values and then make the corresponding current and voltage calculations
		// for each reistor
		for (int i = 1; i <= multiplier; i++){
			for (int j = 1; j <= multiplier; j++){
				...
				lineOut << setprecision(3) << "Loop current: I = " << (vs/(currR1+currR2)) << " Amperes." << endl;
		...
		// sends message to the outputFile indicating that the user has selected a "Divider" file
		lineOut << "-----" << endl << "Circuit #1 (Wheatstone Bridge)" << endl;
		
		// for loops to handle changing the values of resistors for the wheatstone bridge calculations
		for (int i = 1; i <= multiplier; i++){
			for (int j = 1; j <= multiplier; j++){
				for (int k = 1; k <= multiplier; k++){
					for (int m = 1; m <= multiplier; m++){
						for (int n = 1; n <= multiplier; n++){
							...
							// delcared and initialized variables for temporary resistor values that change with each iteration.
							int r1 = r_1*i;
							int r2 = r_2*j;
							int r3 = r_3*k;
							int r4 = r_4*m;
